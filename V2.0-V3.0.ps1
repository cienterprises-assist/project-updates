...
function Load-UserHive {
    ...
    try {
        $regKey = "HKU:\${Sid}"
        if (Test-Path $regKey) {
            Log-Message "Registry hive for ${Username} (${Sid}) already loaded"
            return $true
        }
        $profilePath = (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList\${Sid}" -ErrorAction Stop).ProfileImagePath
        $ntuserPath = Join-Path $profilePath "NTUSER.DAT"
        if (-not (Test-Path $ntuserPath)) {
            throw "NTUSER.DAT not found for ${Username} at $ntuserPath"
        }
        Log-Message "Loading registry hive for ${Username} (${Sid}) from $ntuserPath"
        Start-Process -FilePath "reg" -ArgumentList "load HKU\${Sid} `"$ntuserPath`"" -Wait -ErrorAction Stop
        if (-not (Test-Path $regKey)) {
            throw "Failed to load registry hive for ${Username}"
        }
        Log-Message "Successfully loaded registry hive for ${Username}"
        return $true
    } catch {
        Log-Message "Error loading registry hive for ${Username} (${Sid}) - $_"
        return $false
    }
}
...
function Unload-UserHive {
    ...
    try {
        $regKey = "HKU:\${Sid}"
        if (Test-Path $regKey) {
            Log-Message "Unloading registry hive for ${Username} (${Sid})"
            Start-Process -FilePath "reg" -ArgumentList "unload HKU\${Sid}" -Wait -ErrorAction Stop
            Log-Message "Successfully unloaded registry hive for ${Username}"
        }
    } catch {
        Log-Message "Error unloading registry hive for ${Username} (${Sid}) - $_"
    }
}
...
function Set-Desktop-Wallpaper {
    ...
    try {
        ...
        if (-not (Load-UserHive -Sid $UserSid -Username $Username)) {
            throw "Failed to load user hive for ${Username}"
        }
        $regKey = "HKU:\${UserSid}\Control Panel\Desktop"
        if (-not (Test-Path $regKey)) {
            throw "Registry key $regKey not found for ${Username}"
        }
        ...
        Unload-UserHive -Sid $UserSid -Username $Username
    }
}
