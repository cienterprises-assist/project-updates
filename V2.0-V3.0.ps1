...
function Set-Desktop-Wallpaper {
    ...
    try {
        ...
        Log-Message "Set desktop wallpaper to $Path for user $UserSid"
        Show-Popup "Desktop wallpaper set to $Path for user" "Wallpaper Set"
    } catch {
        Log-Message "Error setting desktop wallpaper for $UserSid : $_"
        Show-Popup "Error setting desktop wallpaper. Check log at: $logPath" "Error" 48
    } finally {
    ...
    foreach ($folder in $FolderNames) {
        ...
        try {
            if (Test-Path $path) {
                ...
                Log-Message "Deleted folder $path"
...
if ($adminWallpaper) {
    ...
    if ($adminWallpaperPath) {
        # Assuming Super Administrator and Agency Admin SIDs (replace with actual SIDs)
        $adminSids = @("S-1-5-21-*-500", "S-1-5-21-*-1001") # Replace with actual SIDs
        foreach ($sid in $adminSids) {
            Set-Desktop-Wallpaper -Path $adminWallpaperPath -UserSid $sid
        }
    }
}
...
if ($userWallpaper) {
    ...
    if ($userWallpaperPath) {
        ...
        Set-Desktop-Wallpaper -Path $userWallpaperPath -UserSid ".DEFAULT"
    }
}
...
$foldersToDelete = @("Folder1 4K", "Folder2 Screen", "Folder3 Wallpaper")
