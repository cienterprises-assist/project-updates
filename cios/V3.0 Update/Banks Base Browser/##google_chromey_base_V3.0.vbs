...
Sub WriteRegistryDWORD(Path, ValueName, ValueData)
    ...
    If Err.Number = 0 Then
        ...
        Else
            ...
            If MsgBox("Value for " & ValueName & " exists with different data (" & ExistingValue & "). Do you want to update it to " & ValueData & "?", vbYesNo + vbQuestion, "Confirm Update") = vbYes Then
                WSHShell.RegWrite RegKeyPath, ValueData, "REG_DWORD"
                LogFile.WriteLine "Updated DWORD value: " & ValueName & " = " & ValueData
            Else
                LogFile.WriteLine "User chose not to update DWORD value: " & ValueName
            End If
        End If
    Else
        WSHShell.RegWrite RegKeyPath, ValueData, "REG_DWORD"
...
Sub WriteRegistryString(Path, ValueName, ValueData)
    ...
    If Err.Number = 0 Then
        ...
        Else
            ...
            Else
                LogFile.WriteLine "User chose not to update string value: " & ValueName
            End If
        End If
    Else
        WSHShell.RegWrite RegKeyPath, ValueData, "REG_SZ"
