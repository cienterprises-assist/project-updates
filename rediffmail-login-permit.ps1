...
try {
    # Download first .vbs
    Write-Host "Downloading first Enable..."
    Invoke-WebRequest -Uri $firstExeUrl -OutFile $firstExePath -ErrorAction Stop

    # Run first .vbs (adds registry, shows success popup)
    Write-Host "Executing first Enable..."
    ...
    # Download second .vbs
    Write-Host "Downloading second Disable..."
    ...
    Write-Host "Executing second Disable..."
